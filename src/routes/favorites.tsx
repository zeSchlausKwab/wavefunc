...
import { Heart, Trash2, Plus, Edit } from "lucide-react";
...
function Favorites() {
  ...
  const currentList = selectedList 
    ? favoritesLists.find(list => list.favoritesId === selectedList)
    : defaultList;
    
  const { stations, isLoading: stationsLoading } = useFavoriteStations(currentList);
  ...
  if (favoriteCount === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <p className="text-muted-foreground text-center max-w-md">
          Start adding stations to your favorites by clicking the heart icon on
          any station card.
        </p>
      </div>
    );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              {currentList.description && (
                <p className="text-muted-foreground text-sm">{currentList.description}</p>
              )}
            </div>
